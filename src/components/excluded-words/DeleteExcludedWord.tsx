...
const DeleteExcludedWord = ({
  ...
  const deleteExcludedWord = useMutation(
    api.excludedWords.mutations.deleteExcludedWord
  )

  const handleDelete = async () => {
    if (!excludedWord) return

    setIsLoading(true)

    try {
      await deleteExcludedWord({ _id: excludedWord._id })
      ...
      >
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            ...
            {`"`}
            <strong className="text-foreground">{excludedWord?.word}</strong>
            {`"`}.
          </AlertDialogDescription>
        </AlertDialogHeader>

        <DeleteConfirmation
          label="Yes, delete the word"
...
export default DeleteExcludedWord
